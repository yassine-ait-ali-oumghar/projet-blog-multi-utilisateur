<template>
  <div class="p-8 rounded-3xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-2xl shadow-slate-200/50 dark:shadow-none animate-slide-up max-w-md w-full mx-auto relative overflow-hidden transition-colors duration-300">
    ...
    <div class="relative text-center mb-10 z-10">
      ...
      <p class="text-slate-500 dark:text-slate-400 mt-2 font-medium">Connectez-vous à votre compte</p>
    </div>

    <!-- Error Alert -->
    <div v-if="errorMessage" class="mb-6 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-2xl p-4 text-sm text-red-700 dark:text-red-400 flex items-center gap-3 relative z-10">
    ...
    <form @submit.prevent="handleLogin" class="space-y-6 relative z-10">
      ...
      <button 
        type="submit" 
        class="w-full bg-primary-600 hover:bg-primary-700 text-white font-bold py-4 rounded-2xl shadow-lg shadow-primary-500/30 transition-all active:scale-[0.98] disabled:opacity-50 flex justify-center items-center gap-2"
        :disabled="isLoading"
      >
    ...
    <div class="mt-10 text-center text-sm text-slate-500 dark:text-slate-400 relative z-10 border-t border-slate-100 dark:border-slate-800 pt-8">
       ...
       <router-link to="/register" class="font-bold text-primary-600 hover:text-primary-800 dark:hover:text-primary-400 transition-colors">Créer un compte</router-link>
